...
function buildQueriesHref({ page, range, from, to, status, q }) {
  ...
  return qs ? `/shiv_admin/queries?${qs}` : '/shiv_admin/queries';
}
...
export default async function AdminQueriesPage({ searchParams }) {
  ...
  if (q) {
    ...
    conditions.push(
      `(name LIKE $${start} OR email LIKE $${start + 1} OR phone LIKE $${start + 2} OR message LIKE $${start + 3} OR COALESCE(service_slug, '') LIKE $${start + 4})`
    );
  }
  ...
  const countResult = await query(
    `SELECT COUNT(*) AS count FROM contact_queries ${whereSql}`,
    filterParams
  );
    ...
    <div className="p-6 lg:p-8">
      <div className="mb-4">
        <p className="text-sm text-slate-500">
          {total === 0
            ? 'No matching queries'
            : `Showing ${fromIdx}–${toIdx} of ${total} quer${total === 1 ? 'y' : 'ies'}`}
        </p>
      </div>
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex flex-col items-center justify-between gap-3 sm:flex-row">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={buildQueriesHref({ ...hrefArgs, page: page - 1 })}
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((item) =>
                typeof item === 'string' ? (
                  ...
                  <Link
                    key={item}
                    href={buildQueriesHref({ ...hrefArgs, page: item })}
