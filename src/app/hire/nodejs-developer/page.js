import Image from 'next/image';
import Link from 'next/link';
import HireHeroOverlay from '@/components/hire/shared/HireHeroOverlay';
import HireWhySplit from '@/components/hire/layout/HireWhySplit';
import HireSkillsGrid from '@/components/hire/layout/HireSkillsGrid';
import HireProcessPanel from '@/components/hire/layout/HireProcessPanel';
import HireEngagementCards from '@/components/hire/layout/HireEngagementCards';
import HireTechStack from '@/components/hire/layout/HireTechStack';
import HireInquiryPanel from '@/components/hire/layout/HireInquiryPanel';
import HireFaqAccordion from '@/components/hire/layout/HireFaqAccordion';
import { buildEngagementModels, buildProcessSteps } from '@/lib/hirePageContent';
import { nodejsTechnologies } from '@/components/hire/nodejs/nodejsTechnologies';
import { getHireHeroImage } from '@/lib/serviceHeroImages';
import { hireAuditMetadata } from '@/lib/auditMetadata';

export const metadata = hireAuditMetadata('nodejs-developer');


const HERO_IMAGE = getHireHeroImage('nodejs-developer');

const whyHireContent = {
  title: 'Why Hire Node.js Developers From Nexuron Technologies?',
  imageAlt: 'Nexuron Node.js development team',
  description:
    'Behind every high performing Node.js application is a team that obsesses over the details most developers skip architecture, scalability, security, and long term code health. That\'s the standard we hold ourselves to, on every project, for every client.',
  benefits: [
    'Experienced Node.js Backend Developers',
    'Fast Development with Modern JavaScript',
    'RESTful & GraphQL API Expertise',
    'Real-Time Application Development',
    'Scalable Microservices Architecture',
    'End-to-End Integration Support',
  ],
};

const expertiseContent = {
  expertiseTitle: 'Our Node.js Development Expertise',
  expertise: [
    {
      title: 'Custom Node.js Backend Development',
      description:
        'Build robust, scalable server-side applications tailored to your business requirements.',
    },
    {
      title: 'RESTful & GraphQL API Development',
      description:
        'Develop secure, high-performance APIs for web and mobile applications.',
    },
    {
      title: 'Real-Time Application Development',
      description:
        'Build real-time systems with WebSockets, Socket.io, and event-driven architecture.',
    },
    {
      title: 'Microservices Architecture',
      description:
        'Design and implement scalable microservices with Node.js for enterprise applications.',
    },
    {
      title: 'Node.js Performance Optimization',
      description:
        'Improve application speed, memory usage, and overall system efficiency.',
    },
    {
      title: 'Application Maintenance & Support',
      description:
        'Ensure long-term stability with version upgrades, bug fixes, and ongoing technical support.',
    },
  ],
};

const faqs = [
  {
    question: 'How quickly can I hire a Node.js developer?',
    answer:
      'You can onboard a dedicated Node.js developer within 48-72 hours after requirement discussion.',
  },
  {
    question: 'Do your developers work in my time zone?',
    answer:
      'Yes, our developers are flexible and can overlap with your time zone to ensure seamless communication, daily reporting, and agile collaboration.',
  },
  {
    question: 'Can I interview the developer before hiring?',
    answer:
      'Absolutely. We provide you with a shortlist of pre-vetted candidates, and you can interview them to ensure they are the perfect technical and cultural fit for your team.',
  },
  {
    question: 'Do you provide post-launch support?',
    answer:
      'Yes, we offer comprehensive post-launch support and maintenance packages to keep your application secure, updated, and performing optimally.',
  },
];




const PROCESS_STEPS = buildProcessSteps('Node.js');

const ENGAGEMENT_MODELS = buildEngagementModels('Node.js', 'Node.js Developer');

export default function HireNodejsDeveloperPage() {
  return (
    <main>
      <section className="relative overflow-hidden min-h-[560px] md:min-h-[620px]">
        <Image
          src={HERO_IMAGE}
          alt="Hire Node.js developers"
          fill
          priority
          className="object-cover object-center"
        />

        <HireHeroOverlay />

        <div className="relative z-10 max-w-[1340px] mx-auto px-4 sm:px-6 lg:px-8 min-h-[560px] md:min-h-[620px] flex items-center pt-12 pb-12">
          <div className="w-full max-w-xl text-white">
            <h1 className="text-3xl md:text-4xl lg:text-[40px] font-bold italic tracking-wide leading-tight">
              Hire Node.js Development Experts
            </h1>
            <p className="mt-5 text-[13px] md:text-[14px] font-normal leading-7 text-gray-100">
              Build fast, scalable, and high-performance backend systems with our expert Node.js developers. We specialize in creating efficient server-side applications, real-time systems, and APIs that power modern web and mobile applications.
            </p>
            <p className="mt-4 text-[13px] md:text-[14px] font-normal leading-7 text-gray-100">
              From startups to enterprise platforms, our team ensures speed, scalability, and reliability to deliver powerful digital solutions tailored to your business needs.
            </p>

            <Link
              href="#hire-inquiry"
              className="inline-flex mt-8 px-8 py-3 rounded-md bg-[#5b9cff] hover:bg-[#4a8bee] text-white text-[15px] font-medium transition-colors"
            >
              Hire Now
            </Link>
          </div>
        </div>
      </section>

      <HireWhySplit
        bg="white"
        title={whyHireContent.title}
        description={whyHireContent.description}
        benefits={whyHireContent.benefits}
        imageAlt={whyHireContent.imageAlt}
      />

      <HireSkillsGrid
        bg="muted" skills={expertiseContent.expertise} />

      <HireTechStack
        bg="white"
        title="Technologies Our Node.js Developers Use"
        technologies={nodejsTechnologies}
      />

      <HireProcessPanel
        bg="muted" title="How Hiring a Node.js Developer Works" steps={PROCESS_STEPS} />

      <HireEngagementCards
        bg="white" models={ENGAGEMENT_MODELS} />

      <HireInquiryPanel
        bg="muted" roleTitle="Node.js Developer" serviceName="Node.js Development" />

      <HireFaqAccordion
        bg="white" faqs={faqs} />
    </main>
  );
}
